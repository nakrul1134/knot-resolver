...
local function parse(rr)
    ...
    local major, minor, patch = package_version()
    if latestVersion ~= string.format('%d.%d.%d', major, minor, patch) then
        log('[version] Current version of Knot DNS Resolver is different from the latest stable one available.'
			.. ' (Current: %d.%d.%d, Latest stable: %s)', major, minor, patch, latestVersion)
		if CVE ~= 'N/A' then
